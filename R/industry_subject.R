...
downloadcrosstabs <- function(subjectinput, YAGinput, countinput, qualinput) {
  ...
  if (countinput == "current_region") {
    crosstabs_data <- tables_data %>%
      ...
      select(current_region, SECTIONNAME, n = count) %>%
    ...
    footer_data <- tables_data %>%
      ...
      select(current_region, subject_name, n = count) %>%
      spread(current_region, n) %>%
      arrange(-All) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. <= 2, 0, .))) %>%
  ...
  if (countinput == "FSM") {
    crosstabs_data <- tables_data %>%
      ...
      select(FSM, SECTIONNAME, n = count) %>%
      spread(FSM, n) %>%
      arrange(-All) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. <= 2, 0, .))) %>%
  ...
  if (countinput == "sex") {
    crosstabs_data <- tables_data %>%
      ...
      select(sex, SECTIONNAME, n = count) %>%
      spread(sex, n) %>%
      arrange(-`F+M`) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. <= 2, 0, .))) %>%
  ...
  if (countinput == "prior_attainment") {
    crosstabs_data <- tables_data %>%
      ...
      select(prior_attainment, SECTIONNAME, n = count) %>%
  ...
  if (countinput == "subject_name") {
    crosstabs_data <- tables_data %>%
      ...
      select(subject_name, SECTIONNAME, n = count) %>%
...
backwards_crosstabs <- function(sectioninput, YAGinput, countinput, qualinput, buttoninput) {
  ...
  if (countinput == "ethnicity") {
    crosstabs_base_data <- tables_data %>%
      filter(
        sex == "F+M", SECTIONNAME == sectioninput, YAG == YAGinput,
        FSM == "All", current_region == "All", prior_attainment == "All",
        qualification_TR == "First degree", subject_name != "All",
        threshold == "All", group_name == "All"
      )
    crosstabs_data <- crosstabs_base_data %>%
      select(ethnicity, subject_name, n = count) %>%
      spread(ethnicity, n) %>%
      arrange(-All) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. <= 2, 0, .))) %>%
    ...
    crosstabs_earnings_data <- crosstabs_base_data %>%
      select(ethnicity, subject_name, n = earnings_median) %>%
      spread(ethnicity, n) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. == 0, NA, .))) %>%
      mutate_at(
    ...
    footer_data <- crosstabs_base_data %>%
      select(ethnicity, subject_name, n = count) %>%
      spread(ethnicity, n) %>%
      arrange(-All) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
  ...
  if (countinput == "current_region") {
    crosstabs_base_data <- tables_data %>%
      filter(
        sex == "F+M", SECTIONNAME == sectioninput, YAG == YAGinput,
        ethnicity == "All", FSM == "All", prior_attainment == "All",
        qualification_TR == "First degree", subject_name != "All",
        threshold == "All", group_name == "All"
      )
    crosstabs_data <- crosstabs_base_data %>%
    ...
    crosstabs_earnings_data <- crosstabs_base_data %>%
      select(current_region, subject_name, n = earnings_median) %>%
      spread(current_region, n) %>%
      arrange(-All) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. == 0, NA, .))) %>%
  ...
  if (countinput == "FSM") {
    crosstabs_base_data <- tables_data %>%
      filter(
        SECTIONNAME == sectioninput, YAG == YAGinput, ethnicity == "All",
        current_region == "All", sex == "F+M", prior_attainment == "All",
        qualification_TR == qualinput, subject_name != "All",
        threshold == "All", group_name == "All"
      )
    crosstabs_data <- crosstabs_base_data %>%
    ...
    crosstabs_earnings_data <- crosstabs_base_data %>%
      select(FSM, subject_name, n = count) %>%
      spread(FSM, n) %>%
      arrange(-All) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. == 0, NA, .))) %>%
  ...
  if (countinput == "sex") {
    crosstabs_data <- tables_data %>%
      filter(
        SECTIONNAME == sectioninput, YAG == YAGinput, ethnicity == "All",
        current_region == "All", FSM == "All", prior_attainment == "All",
        qualification_TR == qualinput, subject_name != "All",
        threshold == "All", group_name == "All"
      ) %>%
      select(sex, subject_name, n = count) %>%
      spread(sex, n) %>%
      arrange(-`F+M`) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. <= 2, 0, .))) %>%
    ...
    crosstabs_earnings_data <- tables_data %>%
      ...
      select(sex, subject_name, n = earnings_median) %>%
      spread(sex, n) %>%
      arrange(-`F+M`) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. == 0, NA, .))) %>%
  ...
  if (countinput == "prior_attainment") {
    crosstabs_base_data <- tables_data %>%
      filter(
        SECTIONNAME == sectioninput, YAG == YAGinput, ethnicity == "All",
        current_region == "All", FSM == "All", sex == "F+M",
        qualification_TR == qualinput, subject_name != "All",
        threshold == "All", group_name == "All"
      )
    crosstabs_data <- crosstabs_base_data %>%
      select(prior_attainment, subject_name, n = count) %>%
      spread(prior_attainment, n) %>%
      arrange(-All) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. <= 2, 0, .))) %>%
    ...
    crosstabs_earnings_data <- crosstabs_base_data %>%
      select(prior_attainment, subject_name, n = earnings_median) %>%
      spread(prior_attainment, n) %>%
      arrange(-All) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. == 0, NA, .))) %>%
  ...
  if (countinput == "SECTIONNAME") {
    crosstabs_base_data <- tables_data %>%
      filter(
        prior_attainment == "All", YAG == YAGinput, ethnicity == "All",
        current_region == "All", FSM == "All", sex == "F+M",
        qualification_TR == qualinput, subject_name != "All",
        threshold == "All", group_name == "All"
      )
    ...
    crosstabs_data <- crosstabs_base_data %>%
      select(SECTIONNAME, subject_name, n = count) %>%
    ...
    crosstabs_earnings_data <- crosstabs_base_data %>%
      select(SECTIONNAME, subject_name, n = earnings_median) %>%
      spread(SECTIONNAME, n) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. == 0, NA, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(!is.na(as.numeric(.)), round(as.numeric(.), -2), .)))
  ...
  if (countinput == "qualification_TR") {
    crosstabs_base_data <- tables_data %>%
      filter(
        sex == "F+M", SECTIONNAME == sectioninput, YAG == YAGinput,
        ethnicity == "All", FSM == "All", current_region == "All",
        prior_attainment == "All", subject_name != "All", threshold == "All",
        group_name == "All"
      )
    crosstabs_data <- crosstabs_base_data %>%
      select(qualification_TR, subject_name, n = count) %>%
    ...
    crosstabs_earnings_data <- crosstabs_base_data %>%
      select(qualification_TR, subject_name, n = earnings_median) %>%
      spread(qualification_TR, n) %>%
      arrange(-`First degree`) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), 0, .))) %>%
      mutate_at(vars(-group_cols()), funs(ifelse(. == 0, NA, .))) %>%
