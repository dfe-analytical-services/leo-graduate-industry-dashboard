...
sankey_table <- function(subjectinput, sexinput, qualinput){
  ...
  # list giving column formatting (using style function) for single column
    ...
    coldefs <- list(INDUSTRY = colDef(na = 'c', name = 'Industry', width = 600, footer = 'TOTAL (N)'),
  ...
  # create table
  sankey_table <- reactable(yag_table_final, defaultPageSize = 21, columns = coldefs, showSortable = TRUE,
...
sankeytext1 <- function(subjectinput, sexinput, qualinput){
  ...
  ifelse(subjectinput=='All',
         ...
         , sep = ''),
         sankeytext1 <- paste("For ", sextext,", ",qualinput , " graduates who studied",subjectinput,", the industry with the highest 
                      proportion of graduates one year after graduation is ", first(yag_table_final$SECTIONNAME.x),
                              " (",first(yag_table_final$`1 YAG`),"%), and at five years after graduation it is ",
                              ifelse(first(five_yag_table_subject$SECTIONNAME.x)==first(yag_table_final$SECTIONNAME.x),
                                     "the same",
...
earnings_sankey <- function(subjectinput, sexinput, earningsinput){
  ...
  sankeyNetwork(Links = links1, Nodes = nodes,
                Source = "source", Target = "target",
...
earnings_table <- function(subjectinput, sexinput, earningsinput){
  ...
  yag_table_final2 <- cohort_earnings1[,c("SECTIONNAME.x", "earnings_median_1YAG", "SECTIONNAME.y",
  ...
  coldefs$earnings_change_average <- colDef(style = function(value) {
  ...
  # create table
  earnings_table <- reactable(yag_table_final2, defaultPageSize = 21, columns = coldefs, showSortable = TRUE, 
                              columnGroups = list(
...
map_chart <- function(sectionnameinput, subjectinput, countinput, YAGinput, qualinput){
  ...
  p_popup <- paste('<B>',leafletmapdata$rgn19nm,'</B>', br(), br(),
...
map_text2 <- function(sectionnameinput, subjectinput, countinput, YAGinput, qualinput){
  ...
  map_text <- paste(subjecttext,'in the', sectionnameinput, 'industry, the region with the highest proportionate increase in graduates who studied there compared to living 
...
maptable <- function(sectionnameinput, subjectinput, countinput, YAGinput, regioninput, qualinput){
  ...
  maptabledata <- mapdata[,c('region', 'trained_in_region2', 'living_in_region2', 'difference2', 'difference_prop2', 'number_of_providers',
                        'earnings_median')]
  
  map_table <- reactable(maptabledata, sortable = TRUE, resizable = TRUE, showSortable = TRUE, 
                         ...
                         columns = list(
                           ...
                           number_of_providers = colDef(name = 'Number of providers', style = list(backgroundColor = "#f7f7f7"),
                           ...
                           earnings_median = colDef(name = 'Median earnings',
...
regional_sankey <- function(sectionnameinput, subjectinput, YAGinput, qualinput){
  ...
  sankey_data <- regional_movement_data %>%
    filter(SECTIONNAME == sectionnameinput, subject_name == subjectinput, YAG == YAGinput, 
...
crosstab_text <- function(subjectinput, YAGinput, countinput, qualinput, thresholdinput){
  ...
  if(countinput == 'sex'){
    ...
    ifelse(first(crosstabs_data$diff, order_by = -crosstabs_data$abs) > 0,
           sextext <- paste('the proportion of male graduates is ',round(first(crosstabs_data$abs, order_by = -crosstabs_data$abs)*100,digits=1),
                            ' percentage points higher than the proportion of female graduates.'),
           sextext <- paste('the proportion of female graduates is ',round(first(crosstabs_data$abs, order_by = -crosstabs_data$abs)*100,digits=1),
    ...
    ifelse(first(crosstabs_earnings_data$diff, order_by = -crosstabs_earnings_data$abs) > 0,
           sextextearnings <- paste('the median earnings of male graduates were £',
                            ...
                            '  higher than the medain earnings of female graduates.', sep = ''),
           sextextearnings <- paste('the median earnings of female graduates were £',
                                    format(first(crosstabs_earnings_data$abs, order_by = -crosstabs_earnings_data$abs), big.mark = ",", scientific = FALSE),
                            '  higher than the median earnings of male graduates.', sep = '')
    )
    ...
    ifelse(first(crosstabs_earnings_data$Male, order_by = -crosstabs_earnings_data$Male) > first(crosstabs_earnings_data$Female, order_by = -crosstabs_earnings_data$Female),
           ...
           (median earnings of £',format(first(crosstabs_earnings_data$Male, order_by = -crosstabs_earnings_data$Male), big.mark = ",", scientific = FALSE),').', sep = ''),
  ...
  if(countinput == 'FSM'){
    ...
    ifelse(first(crosstabs_data$diff, order_by = -crosstabs_data$abs) >0,
           FSMtext <- paste('the proportion of non-FSM graduates is',round(first(crosstabs_data$abs, order_by = -crosstabs_data$abs)*100,digits=1),
                            'percentage points higher than the proportion of FSM graduates.'),
           FSMtext <- paste('the proportion of FSM graduates is',round(first(crosstabs_data$abs, order_by = -crosstabs_data$abs)*100,digits=1),
    ...
    ifelse(first(crosstabs_earnings_data$diff, order_by = -crosstabs_earnings_data$abs) > 0,
           FSMearningstext <- paste('the median earnings of non-FSM graduates were £',
                                    format(first(crosstabs_earnings_data$abs, order_by = -crosstabs_earnings_data$abs), big.mark = ",", scientific = FALSE),
                                    '  higher than the medain earnings of FSM graduates.', sep = ''),
           FSMearningstext <- paste('the median earnings of FSM graduates were £',
                                    format(first(crosstabs_earnings_data$abs, order_by = -crosstabs_earnings_data$abs), big.mark = ",", scientific = FALSE),
                                    '  higher than the median earnings of non-FSM graduates.', sep = ''))
  ...
  if(countinput == 'ethnicity'){
    ...
    ethnicityfirstdata <- c(ethnicityfirst('White'), ethnicityfirst('Black'), ethnicityfirst('Asian'),
    ...
    if(length(uniqueethnicity)==1){
      ...
      ethnicitytext <- paste(uniqueethnicity[1],' was the most common industry for ',textprod(data1),' ethnicity graduates, 
 ...
 crosstabs_earnings_data2[result[1],result[2]]
  ...
  if(countinput == 'current_region'){
    ...
    regionfirstdata <- c(regionfirst('North East'), regionfirst('North West'), regionfirst('Yorkshire and the Humber'), regionfirst('East Midlands'),
    ...
    regionfirstdata$region <- c('the North East', 'the North West', 'Yorkshire and the Humber', 'the East Midlands', 'the West Midlands',
  ...
  if(countinput == 'prior_attainment'){
    ...
    grad_numbers <- c(sum(footer_data$`1`), sum(footer_data$`2`), sum(footer_data$`3`), sum(footer_data$`4`), 
                      sum(footer_data$`5`),sum(footer_data$`6`), sum(footer_data$`7`), sum(footer_data$`8`),
                      sum(footer_data$`9`))
   
    grad_numbers <- data.frame(grad_numbers)
    grad_numbers$prior_attainment <- c('4 As or more', '360 points', '300-359 points', '240-299 points', '180-239 points', 'Below 180 points', '1 or 2 A level passes',
                                       'BTEC', 'Other')
    ...
    names(crosstabs_earnings_data3) <- c('4 As or more', '360 points', '300-359 points', '240-299 points', '180-239 points', 'Below 180 points', '1 or 2 A level passes',
  ...
  if(countinput == 'qualification_TR'){
    ...
    qualfirstdata <- c(qualfirst('First degree'), qualfirst('Level 7 (taught)'), qualfirst('Level 7 (research)'), 
                       qualfirst('Level 8'))
...
backwards_crosstabs <- function(sectioninput, YAGinput, countinput, qualinput, buttoninput){
  ...
  if(countinput == 'SECTIONNAME'){
    crosstabs_data <- tables_data %>%
      filter(sex == 'F+M', YAG == YAGinput, ethnicity == 'All', FSM == 'All', current_region == 'All', prior_attainment == 'All', 
    ...
    coldefs$subject_name <- colDef(name = 'Subject area', width = 600, footer = 'TOTAL (N)', 
                                  style = list(position = "sticky", left = 0, background = "#fff", zIndex = 1),
                                  headerStyle = list(position = "sticky", left = 0, background = "#fff", zIndex = 1),
                                  footerStyle = list(position = "sticky", left = 0, background = "#fff", zIndex = 1, fontWeight = 'bold'))
  ...
  crosstab <- reactable(crosstabs_data, defaultPageSize = 37, showSortable = TRUE, columns = coldefs,
                        defaultColDef = colDef(footerStyle = list(fontWeight = 'bold')))
